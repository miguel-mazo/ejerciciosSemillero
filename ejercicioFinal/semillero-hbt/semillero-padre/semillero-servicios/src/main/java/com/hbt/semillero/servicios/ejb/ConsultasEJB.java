...
public class ConsultasEJB implements IConsultasEjbLocal {
	...
	/**
	 * {@link com.hbt.semillero.servicios.interfaces.IConsultasEjbLocal#consultaMarcarLinea(Long, Long)}
	 */
	...
	public List<VehiculoDTO> consultarMarcaLinea(Long idMarca, Long idLinea){
		...
		String jpql = "SELECT v FROM Vehiculo v JOIN FETCH v.linea l JOIN FETCH l.marca m "
				+ "WHERE (l.idLinea = :idLinea OR -1 = :idLinea) AND "
					  + "(m.idMarca = :idMarca OR -1 = :idMarca)";
		
		@SuppressWarnings("unchecked")
		List<Vehiculo> vehiculos = em.createQuery(jpql)
	...
	/**
	 * {@link com.hbt.semillero.servicios.interfaces.IConsultasEjbLocal#editarVehiculo(VehiculoDTO)}

	 */
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void editarVehiculo(VehiculoDTO vehiculoDTO) {
		Vehiculo vehiculo = consultarVehiculo(vehiculoDTO.getIdVehiculo());
	...
	/**
	 * {@link com.hbt.semillero.servicios.interfaces.IConsultasEjbLocal#borrarVehiculo(Long)}

	 */
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void borrarVehiculo(Long idVehiculo) {
		Vehiculo vehiculo = consultarVehiculo(idVehiculo);
		em.remove(vehiculo);
	}
	
	/**
	 * {@link com.hbt.semillero.servicios.interfaces.IConsultasEjbLocal#consultarVehiculo(Long)}
	 */
	@Override
	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public Vehiculo consultarVehiculo(Long idVehiculo) {
		List<Vehiculo> vehiculo = em
				.createQuery("Select v from Vehiculo v WHERE v.idVehiculo = :idVehiculo ")
				.setParameter("idVehiculo", idVehiculo).getResultList();
